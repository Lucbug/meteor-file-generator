...
export default class MeteorFileGeneratorView {
  ...
  constructor(serializedState) {
    ...
    atom.packages.onDidActivatePackage(function () {
      dir = path.dirname(atom.packages.getActivePackage('tree-view').mainModule.provideTreeView().selectedPaths()[0]);
    });
    ...
    try {
      if (/\/$/.test(pathToCreate)) {
            mkdirp(pathToCreate, function (err) {
                if (err) { atom.notifications.addError("Error : " + err) }
            });
            } else {
              if (createFileInstantly) {
                if (!fs.existsSync(createWithin) || !fs.statSync(createWithin, function (err) {
                  atom.notifications.addInfo('Folder does not exist. Creating it.')
                })) { mkdirp(createWithin); };
